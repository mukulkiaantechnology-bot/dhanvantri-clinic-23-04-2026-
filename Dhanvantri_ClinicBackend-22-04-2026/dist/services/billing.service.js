...
const isInterStateSupply = (clinicState, placeOfSupply) => {
...
export const getInvoices = async (clinicId) => {
    ...
    return invoices.map((invoice) => {
        ...
        const remarkItems = (invoice.items || []).filter((item) => String(item.serviceType || '').toLowerCase() === 'remark');
        const billableItems = (invoice.items || []).filter((item) => !['tax', 'remark'].includes(String(item.serviceType || '').toLowerCase()));
        const subtotal = billableItems.reduce((sum, item) => sum + Number(item.amount || 0), 0);
        const totalTax = taxItems.reduce((sum, item) => sum + Number(item.amount || 0), 0);
        // Recover GST rate from stored tax line description: "GST (18%) - CGST+SGST"
        let gstRate = 0;
        const rateMatch = taxItems[0]?.description?.match(/\(([\d.]+)%\)/);
        if (rateMatch)
            gstRate = Number(rateMatch[1] || 0);
...
export const getCorporatePharmacySummary = async (clinicId, month) => {
