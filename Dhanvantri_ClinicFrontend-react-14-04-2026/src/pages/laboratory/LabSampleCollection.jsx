            ...
            <div className="content-section mt-xl">
                ...
                return (<tr key={order.id}>
                                                ...
                                                <td style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                                    {!isCollected && (<button type="button" className="btn btn-primary btn-sm btn-with-icon" onClick={() => handleCollectSample(order.id)} disabled={!isPaidStatus} style={{ background: 'linear-gradient(135deg, #2D3BAE 0%, #1E1B4B 100%)', border: 'none', color: 'white' }} title={!isPaidStatus ? "Payment required before collection" : ""}>
                                                            <FiCheckCircle /> Mark Collected
                                                        </button>)}
                                                    <button type="button" className="btn btn-upload-report btn-sm btn-with-icon" onClick={() => handleOpenUpload(order)} disabled={!isCollected} title={!isCollected ? "Sample must be collected before uploading report" : ""}>
